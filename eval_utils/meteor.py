...
def evaluate_meteor_match(ref_data, pred_data):
    """
    ...
    for elem in tqdm(pred_data):
        cur_sys = " ".join(elem["generate"])  # str
        tid = str(elem["prompt_id"]) # 0
        if not tid in ref_data: 
          miss_count += 1
          continue
        cur_ref_lst = [" ".join(elem) for elem in ref_data[tid]]

        if len(cur_ref_lst) > 0:
            best_scrs = round(meteor_score(cur_ref_lst, cur_sys),4)
            agg.append(best_scrs)
            sample_evaluated += 1
            
            if tid in tid_best_score_dict:
                if best_scrs > tid_best_score_dict[tid]:
                    tid_best_score_dict[tid] = best_scrs
            else:
                tid_best_score_dict[tid] = best_scrs
    ...
    agg_best = []
    print("========evaluate best results: ========")
    print("num evaluated: ", len(tid_best_score_dict))
    for tid in tid_best_score_dict:
        agg_best.append(tid_best_score_dict[tid])
    print("best score: ", np.mean(agg_best))


    print("========evaluate mean results: ========")
    print(sample_evaluated)
    print("mean score: ", np.mean(agg))
